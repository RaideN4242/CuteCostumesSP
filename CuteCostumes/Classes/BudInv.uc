...
{ 
   local Inventory I;
	...
	if(Role==ROLE_Authority)
	{
		if(BuMask == none)
		{
			BuMask = Spawn(class'BudMask',Other);
...
defaultproperties
{
     HudImage=Texture'CuteCostumes_T.Bud_Trader'
     SelectedHudImage=Texture'CuteCostumes_T.Bud_Trader'
     Weight=0.000000
     TraderInfoTexture=Texture'CuteCostumes_T.Bud_Trader'
     FireModeClass(0)=Class'KFMod.NoFire'
     FireModeClass(1)=Class'KFMod.NoFire'
     SelectForce="SwitchToAssaultRifle"
     bCanThrow=False
     Description="Costume: Bud Mask."
     Priority=1
     InventoryGroup=5
     GroupOffset=5
     PickupClass=Class'CuteCostumes.BudPickup'
     BobDamping=6.000000
