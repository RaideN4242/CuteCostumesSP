...
{ 
   local Inventory I;
	...
	if(Role==ROLE_Authority)
	{
		if(NMask == none)
		{
			NMask = Spawn(class'NonMask',Other);
			NMask.TransferActor(Other);
		}
...
defaultproperties
{
     HudImage=Texture'CuteCostumes_T.non_trader'
     SelectedHudImage=Texture'CuteCostumes_T.non_trader'
     Weight=0.000000
     TraderInfoTexture=Texture'CuteCostumes_T.non_trader'
     FireModeClass(0)=Class'KFMod.NoFire'
     FireModeClass(1)=Class'KFMod.NoFire'
     SelectForce="SwitchToAssaultRifle"
     bCanThrow=False
     Description="Costume: Non La Hat."
     Priority=1
     InventoryGroup=5
     GroupOffset=5
     PickupClass=Class'CuteCostumes.NonPickup'
     BobDamping=6.000000
