class PandaInv extends MaskInv;
...
{ 
   local Inventory I;
	...
	if(Role==ROLE_Authority)
	{
		if(PMAsk == none)
		{
			PMAsk = Spawn(class'PandaMask',Other);
			PMAsk.TransferActor(Other);
		}
...
defaultproperties
{
     HudImage=Texture'CuteCostumes_T.panda_trader'
     SelectedHudImage=Texture'CuteCostumes_T.panda_trader'
     Weight=0.000000
     TraderInfoTexture=Texture'CuteCostumes_T.panda_trader'
     FireModeClass(0)=Class'KFMod.NoFire'
     FireModeClass(1)=Class'KFMod.NoFire'
     SelectForce="SwitchToAssaultRifle"
     bCanThrow=False
     Description="Costume: Panda Mask."
     Priority=1
     InventoryGroup=5
     GroupOffset=5
     PickupClass=Class'CuteCostumes.PandaPickup'
     BobDamping=6.000000
